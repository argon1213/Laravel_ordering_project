  ...
  const PRODUCTION = 'production';
    ...
    public function orderSubmit(Request $request) {
        ...
        try{
          ...
          $newPayment->order->client->university;
          $newPayment->order->paymentType;
          ...
          $result = PaymentApiController::processPayment($newPayment, $newClient, $data['stripeToken'], route('wechatpay-api-return'), route('alipay-api-return'));
    ...
    public function getOrders(Request $request)
    {
      ...
      $orders = SomsOrder::with('items')->where('client_id', $request->get('client_id'));
      $total = $orders->count();
      if($label == "init") {
        $orders = $orders->orderBy('updated_at','desc')->offset($offset)->limit($limit)->get();
      } else {
        $orders = $orders->orderBy('code', $label)->offset($offset)->limit($limit)->get();
      }
      $currentOrders['orders'] = $orders;
      $currentOrders['page'] = $page;
      $currentOrders['total'] = $total;
